...
const INITIAL_COMMENTS = [
  {
    ...
    text: 'The drone shots in this video are absolutely phenomenal. What gear were you using for those sweeping valley views?',
  ...
  },
  {
    ...
    text: 'This makes me want to pack my bags and leave immediately. Thanks for the inspiration! 🏔️',
  ...
  },
  {
    ...
    text: 'Just what I needed to watch during my lunch break. Subscribed! Keep the amazing content coming.',
  ...
  },
  {
    _id: 'c04',
    userId: 'user_ext4',
    username: 'techpilot_x',
    text: 'Brilliant content as always. The editing quality here is top notch. 10/10 would recommend.',
    createdAt: '2026-04-30T19:45:00Z',
    likes: 34,
    liked: false,
  },
];
...
function CommentItem({ comment, currentUser, onEdit, onDelete }) {
  const isOwn = currentUser && comment.userId === currentUser._id;
    ...
    <div className="flex gap-3 group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          {isOwn && (
            <span className="text-label-md font-label-md text-primary ml-1">
              (you)
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={handleLike}
            className="flex items-center gap-1 hover:bg-surface-variant px-2 py-1 rounded-full transition-colors"
          >
            <span
              className="material-symbols-outlined text-[18px] text-on-surface"
              style={localLiked ? { fontVariationSettings: '"FILL" 1', color: '#bc0100' } : {}}
            >
              thumb_up
            </span>
            <span className="text-body-sm text-secondary">{likeCount > 0 ? likeCount : ''}</span>
          </button>
...
export default function CommentSection({ videoId }) {
  ...
  function handleSaveEdit(id) {
    ...
    setComments(prev =>
      prev.map(c => c._id === id ? { ...c, text: editText.trim() } : c)
    );
    setEditingId(null);
    setEditText('');
  }

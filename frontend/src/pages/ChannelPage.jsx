  ...
  return (
    ...
    >
      ...
      <div className="relative w-full aspect-video bg-surface-container-low rounded-xl overflow-hidden mb-3">
        ...
        {/* Owner edit/delete overlay on hover */}
        {isOwner && (
...
/* ────────────────────────── VideoModal ────────────────────────── */
function VideoModal({ mode, video, onClose, onSave }) {
    ...
    return (
      <div>
        ...
        <El
          ...
          rows={opts.rows}
          className={`w-full px-4 py-2.5 rounded-lg text-body-md text-on-surface bg-surface-container border outline-none transition-colors placeholder:text-secondary resize-none ${
            errors[key]
              ? "border-error"
              : "border-surface-variant focus:border-on-surface"
          }`}
        />
        {errors[key] && (
          <p className="text-error text-body-sm mt-1">{errors[key]}</p>
        )}
      </div>
    );
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-surface-container-lowest rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          {field("Thumbnail URL", "thumbnailUrl", {
            placeholder: "https://...",
          })}
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              className="px-5 py-2.5 bg-on-surface text-surface-container-lowest rounded-full text-body-md font-medium hover:opacity-85 transition-opacity"
            >
...
export default function ChannelPage() {
  ...
  const [videoModal, setVideoModal] = useState(null); // null | { mode, video? }
  const [videos, setVideos] = useState(() =>
    mockVideos.filter((v) => v.channelId === id),
  );

  const channel = mockChannels.find((c) => c._id === id) ?? {
    _id: id,
    channelName: "Channel",
    handle: `@channel`,
    description: "Welcome to this channel.",
    bannerUrl: `https://picsum.photos/seed/${id}b/1280/351`,
    avatarBg: "#4d96ff",
    initial: "C",
    subscribers: 0,
    videoCount: 0,
  ...
  const isOwner = !!(user && user._id === channel.ownerId);

  const sortedVideos = useMemo(() => {
    const v = [...videos];
    if (sortBy === "Latest")
      return v.sort((a, b) => new Date(b.uploadDate) - new Date(a.uploadDate));
    if (sortBy === "Popular") return v.sort((a, b) => b.views - a.views);
    if (sortBy === "Oldest")
      return v.sort((a, b) => new Date(a.uploadDate) - new Date(b.uploadDate));
    return v;
  }, [videos, sortBy]);

  const uploadedCount = videos.filter((v) => v._id.startsWith("v_")).length;
  ...
  function handleSaveVideo(form) {
    if (videoModal.mode === "upload") {
      ...
      setVideos((prev) =>
        prev.map((v) =>
          v._id === videoModal.video._id ? { ...v, ...form } : v,
        ),
      );
    }
    setVideoModal(null);
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex flex-1 relative">
        ...
        <main
          className={`flex-1 min-w-0 transition-all duration-300 ${sidebarOpen ? "md:ml-60" : "md:ml-0"}`}
        >
          {/* ── Channel Banner ── */}
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 items-start sm:items-end pt-3 pb-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-1.5 text-body-sm text-secondary mb-2">
                  ...
                  <span>
                    {formatSubscribers(
                      channel.subscribers + (subscribed ? 1 : 0),
                    )}{" "}
                    subscribers
                  </span>
                  <span>•</span>
                  <span>
                    {(channel.videoCount + uploadedCount).toLocaleString()}{" "}
                    videos
                  </span>
                </div>
                ...
                <div>
                  <p
                    className={`text-body-sm text-secondary max-w-2xl whitespace-pre-wrap ${!descExpanded ? "line-clamp-2" : ""}`}
                  >
                    {channel.description}
                  </p>
                ...
                {channel.links.length > 0 && (
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    ...
                    {channel.links.slice(0, 1).map((link, i) => (
                      <span
                        key={i}
                        className="text-body-sm text-blue-400 hover:underline cursor-pointer"
                      >
                        {link}
                      </span>
                    ))}
                    {channel.links.length > 1 && (
                      <span className="text-body-sm text-secondary">
                        and {channel.links.length - 1} more link
                        {channel.links.length > 2 ? "s" : ""}
                      </span>
                    )}
                  </div>
                )}
                ...
                {isOwner ? (
                  ...
                  <>
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-label-md font-label-md transition-all ${
                        subscribed
            ...
            <div className="flex items-center overflow-x-auto no-scrollbar -mb-px">
              {TABS.map((tab) => (
                <button
                  ...
                  className={`px-4 py-3 text-label-md font-label-md whitespace-nowrap border-b-2 transition-colors ${
                    activeTab === tab
                      ? "border-on-surface text-on-surface"
                      : "border-transparent text-secondary hover:text-on-surface hover:bg-surface-variant/40"
                  }`}
                >
                  {tab}
                </button>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 gap-y-6">
                    {sortedVideos.map((video) => (
                      <ChannelVideoCard
                        ...
                        isOwner={isOwner}
                        onEdit={(v) =>
                          setVideoModal({ mode: "edit", video: v })
                        }
